...
const probeYtDlp = async () => {
    ...
    for (const candidate of candidates) {
        ...
        if (result.code === 0) {
            console.log(
                `======> [expand] yt-dlp runner selected: ${candidate.command} ${candidate.prefixArgs.join(" ")}`.trim(),
            );
            return candidate;
        }
    }
...
const resolveYtDlpRunner = async () => {
    if (!ytDlpRunnerPromise) {
        ytDlpRunnerPromise = probeYtDlp();
    }
...
const buildNetscapeCookieFile = async (cookieHeader = "") => {
    ...
    const tempDir = await mkdtemp(path.join(os.tmpdir(), "expand-ytdlp-cookies-"));
    const filePath = path.join(tempDir, "cookies.txt");
    const expiry = Math.floor(Date.now() / 1000) + 7 * 24 * 3600;

    const lines = [
        "# Netscape HTTP Cookie File",
        "# Generated by cobalt expand youtube playlist helper",
    ];
...
const extractYouTubePlaylistId = (urlString) => {
    ...
    try {
        ...
        return listId?.trim() || undefined;
    } catch {
        return;
    }
};

const isBlockedYouTubePlaylistId = (playlistId) => {
    if (!playlistId) return false;
    return (
        playlistId === "WL" ||
        playlistId.startsWith("RD") ||
        playlistId.startsWith("RL")
    );
};

const normalizeYouTubeEntryUrl = (entry) => {
...
const fetchYouTubePlaylistInfo = async ({ inputUrl, cookieHeader = "" }) => {
    const runner = await resolveYtDlpRunner();
    ...
    try {
        ...
        return parseYtDlpJson(result.stdout);
    } finally {
...
const expandYouTube = async (inputUrl) => {
    const playlistId = extractYouTubePlaylistId(inputUrl);

    if (!playlistId || isBlockedYouTubePlaylistId(playlistId)) {
        return {
            service: "youtube",
            kind: "single",
            items: [{ url: inputUrl }],
    ...
    const attempts = [
        { cookieHeader: "" },
        ...(browserCookieHeader ? [{ cookieHeader: browserCookieHeader }] : []),
    ];

    for (const attempt of attempts) {
        const playlistInfo = await fetchYouTubePlaylistInfo({
            inputUrl,
        ...
        const items = uniqBy(
            entries
                .map((entry) => {
                    const id = toStringId(entry?.id);
                    const url = normalizeYouTubeEntryUrl(entry);
                    if (!url) return null;

                    return {
                        itemKey: id ? `youtube:video:${id}` : undefined,
                        url,
                        title:
        ...
        if (items.length > 1) {
            ...
            const title =
                typeof playlistInfo?.title === "string" && playlistInfo.title.trim()
                    ? playlistInfo.title.trim()
                    : undefined;

            return {
                service: "youtube",
                kind: "youtube-playlist",
                collectionKey: resolvedPlaylistId
                    ? buildCollectionKey("youtube", "playlist", resolvedPlaylistId)
                    : undefined,
                title,
                items,
            };
