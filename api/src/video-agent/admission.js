...
import {estimateRunTtsBudget,getTtsConfig,ttsConfigured} from "./tts-config.js";

export const executionReady = async({dubbing=false}={})=>{
    const { productionHandlers }=await import("./worker-handlers.js");
    return speechConfigured() && (!dubbing || ttsConfigured()) && ["probe","chunk","transcribe","normalize","select_clips","translate_selected",
        ...(dubbing?["prepare_dub_text","tts","fit_dub_timeline","build_dub_subtitles"]:["build_subtitles"]),"render","verify","publish_results"].every(stage=>typeof productionHandlers[stage]==="function");
};
...
export const admitRun = async(client,project,run,retryOf,{ready=executionReady}={})=>{
    ...
    if(!await ready({dubbing:!!run.plan.dubbing?.enabled}))throw agentError("VIDEO_AGENT_PIPELINE_NOT_READY",503,"Video processing is not available yet");
    const now=Date.now();
    ...
    const budget={reservationId:reservation.id,billingRunId:reservation.run_id,reservedSeconds:paid?0:reservation.reserved_seconds,periodKey:reservation.period_key,chargePoint:"before_transcribe",alreadyCommitted:!!paid,
        ...(run.plan.dubbing?.enabled?{dubbing:estimateRunTtsBudget(run.plan.clips,getTtsConfig())}:{})};
