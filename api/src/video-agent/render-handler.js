...
export const renderHandler=async ctx=>{
    ...
    for(const clip of value.clips){
        ...
        try{await runAbortableProcess(ffmpeg,["-nostdin","-v","error","-y","-threads","1","-filter_threads","1","-ss",(clip.startMs/1000).toFixed(3),"-i",input,"-t",((clip.endMs-clip.startMs)/1000).toFixed(3),"-map","0:v:0","-map","0:a:0","-vf","scale=1080:1920:force_original_aspect_ratio=increase,crop=1080:1920,setsar=1,ass=clip.ass","-af","aresample=async=1:first_pts=0,apad","-c:v","libx264","-threads","1","-preset","veryfast","-crf","21","-pix_fmt","yuv420p","-r","30","-c:a","aac","-ar","48000","-b:a","128k","-movflags","+faststart","-fs",String(RENDER_CONFIG.maxBytes),output],{signal,cwd:workDir,timeoutMs:15*60*1000});
