...
export const createRenderHandler=({download=downloadVerifiedSource,runProcess=runAbortableProcess}={})=>async ctx=>{
    ...
    if(hashInput(claim.step.input_snapshot.config)!==hashInput({...RENDER_CONFIG,...claim.run.plan.video,...(dubbed?{dubbing:claim.run.plan.dubbing}:{})}))throw agentError("VIDEO_AGENT_RENDER_CONFIG_CHANGED",409,"Render configuration changed");
    ...
    for(const clip of value.clips){
        ...
        try{await runProcess(ffmpeg,["-nostdin","-v","error","-y","-threads","1","-filter_threads","1","-ss",(clip.startMs/1000).toFixed(3),"-i",input,
            ...
            "-vf",`scale=1080:1920:force_original_aspect_ratio=increase,crop=1080:1920:(in_w-out_w)*${focusX.toFixed(3)},setsar=1,ass=clip.ass`,
