...
精彩片段数量按源视频时长动态限制：60 秒及以下最多 1 个、3 分钟及以下最多 2 个、10 分钟及以下最多 3 个、更长视频最多 5 个。AI 返回至少一个合法片段时直接采用；只有零合法片段时才使用确定性降级算法。候选按时间 IoU 去重，重叠达到 60% 时只保留评分最高者。语音转写不可用时不会把任务伪装为成功。
...
- 长转写段会按照时长和视觉宽度拆为约 1.2–3.5 秒的短字幕，限制每屏文字量并保留 9:16 左右安全边距。
- Worker 镜像安装 Fontconfig、Noto Core 和 Noto CJK 字体，ASS 默认使用 `Noto Sans`。
...
清理 CronJob 每小时执行一次，使用 PostgreSQL `FOR UPDATE SKIP LOCKED` 分批领取资产。删除时使用完整对象 key 和 generation；失败后指数退避重试。用户可在最近任务中删除草稿、已完成、失败或已取消的任务；运行中任务必须先取消。删除后任务立即从列表隐藏，所有源文件、MP4、SRT 和 VTT 登记为待清理。仍活动的 GCS resumable upload session 会立即终止；批量资产继续由 CronJob 异步删除并在失败时重试。删除任务不返还已经提交的会员月度分钟。
...
5. 每个启用片段获得 MP4、SRT、VTT，MP4 分辨率为 1080×1920，烧录字幕位于安全区且长句已拆分。
6. 刷新页面后任务和结果仍存在，并获得新的签名 URL。
7. 完成一次普通站内视频下载后，AI 视频页面能显示并消费最近下载导入凭证。
8. 取消、Provider 临时错误和 Worker 重启时，任务能够取消或从失败阶段重试。
9. 36–60 秒短视频只生成一个高质量候选；高度重叠候选被去重。
10. 结果预览能显示真实时长并播放；页面停留超过 10 分钟或从后台恢复时自动刷新签名链接。
11. 删除终态任务后列表立即移除，下一轮清理确认对应 GCS 资产被删除。
