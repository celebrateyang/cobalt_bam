...
任务 6 实施记录（2026-09-19）：6-1 已接入逐片字幕构建与 SRT/VTT/ASS 导出；6-2 已接入 1080×1920 原音竖屏渲染、逐片检查点恢复、独立媒体校验与结果发布；6-3 已接入结果查询、页面预览和鉴权下载。当前仍使用结构化计划，自然语言规划属于任务 7，配音属于后续任务。生产执行开关默认值未改变；交付协议、测试与部署条件见 [任务 6 说明](video-agent-task6.md)。
...
任务 4-1 已实现真实音频分块处理器：约 600 秒切块、静音切点搜索、前后各 2 秒重叠区、处理与归属时间窗口、逐块持久化及自动重试恢复。分块配置纳入输入 hash，原始媒体流式校验，临时磁盘和实际块大小均受限；时间映射和产物协议详见 [任务 4-1 说明](video-agent-task4-1.md)。本阶段仅接入 `probe → chunk`，转写与后处理分别由任务 4-2、4-3 完成，完整执行入口继续关闭。
...
任务 4-3 已接入 normalize 阶段：保留原始转写，按词边界、停顿、标点和显示宽度生成稳定 cue；保留短语气词及真实重复，明确标记时间估算、轻微抖动修正和可读性疑点。时间线不可安全放入单轨时明确报错。配置、产物协议和处理边界见 [任务 4-3 说明](video-agent-task4-3.md)。完整执行入口待任务 5、6 完成后开放。

任务 4-2 已接入现有转写服务，完成逐块原始结果保存、自动重试恢复、全片时间线合并和边界去重。模型及转写配置纳入输入 hash；缺少词级时间时明确标记 estimated。字幕断句和后处理由任务 4-3 完成，完整执行入口继续关闭。配置、计费和恢复边界见 [任务 4-2 说明](video-agent-task4-2.md)。
...
任务 5-3 已串联验收 normalize、select_clips、translate_selected，覆盖一小时已保存转写、选片限流和格式补救、翻译原始结果提交后中断恢复及跨项目资产拒绝；任务 6 可复用 translated-clips-v1 校验器。自动化验收不替代真实模型语义质量和生产存储验收，完整执行入口继续关闭。协议及验收边界见 [任务 5-3 说明](video-agent-task5-3.md)。

任务 5-2 已接入 translate_selected：仅翻译选中 cue，严格校验 ID 覆盖及非空文本，逐批保存原始输出和成功结果；一次补救后仍失败则拆批，单句失败明确报错。术语表通过计划快照修订，译文继承源时间边界，数字、术语、长度和语言脚本疑点保留标记。实现和边界见 [任务 5-2 说明](video-agent-task5-2.md)。本阶段未增加术语或字幕编辑器，完整执行入口继续关闭。

任务 5-1 已接入 select_clips：长片分窗生成候选，返回稳定 cue ID 范围，服务端映射时间、校验时长并进行全局排序和重叠去重；逐窗保存原始输出及候选，候选不足记录 shortfall。边界上下文和模型输入均受限。实现与恢复说明见 [任务 5-1 说明](video-agent-task5-1.md)。翻译由任务 5-2 接入，完整执行入口继续关闭。
...
| 4 | 长视频分块、逐块转写、词级字幕对齐 | 已实现并通过自动化验收；缺少词时间时标记估算 |
| 5 | 窗口选片、全局去重、上下文翻译与局部恢复 | 已实现并通过集成测试；真实模型质量待验收 |
| 6 | 渲染验收、结果编辑、下载与版本 | 已实现渲染验收、预览与下载；编辑和版本修改并入任务 7-4 |
| 7 | Agent 对话、工具、自动执行、进度与首版上线 | 7-1 会话持久化、7-2 自然语言 Planner、7-3 受控工具与自动执行、7-4 结果编辑与版本交互已实现；见任务 7-4 说明 |
| 8 | TTS、配音时长适配、试听与预算 | 已实施，默认关闭；见 [任务 8 说明](video-agent-task8.md) |
